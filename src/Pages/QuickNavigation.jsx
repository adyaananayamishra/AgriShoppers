import React from "react";
import { FaHome, FaBoxOpen, FaHandshake, FaLeaf } from "react-icons/fa";
import { Link } from "react-router-dom";

const QuickNavigation = () => {
  const links = [
    { icon: <FaHome />, title: "Home", path: "/" },
    { icon: <FaLeaf />, title: "Farmer Resources", path: "/farmer-resources" },
    { icon: <FaBoxOpen />, title: "Products", path: "/products" },
    { icon: <FaHandshake />, title: "Careers", path: "/careers" },
  ];

  return (
    <section className="bg-green-50 py-16 px-6">
      <div className="max-w-4xl mx-auto">
        <h2 className="text-4xl font-extrabold text-green-900 mb-6 relative inline-block">
          Quick Navigation
          <span className="absolute left-0 -bottom-1 w-28 h-1 bg-gradient-to-r from-green-400 to-green-600 rounded-full"></span>
        </h2>
        <p className="text-gray-700 mb-10 text-lg">
          Easily access important sections of our platform with the following quick links:
        </p>

        <div className="grid sm:grid-cols-2 gap-6">
          {links.map((link, index) => (
            <Link
              to={link.path}
              key={index}
              className="flex items-center gap-4 bg-white p-5 rounded-xl shadow hover:shadow-lg transition-all duration-300"
            >
              <div className="p-3 bg-green-100 text-green-600 rounded-full w-12 h-12 flex items-center justify-center text-xl">
                {link.icon}
              </div>
              <span className="text-gray-800 font-medium">{link.title}</span>
            </Link>
          ))}
        </div>
      </div>
    </section>
  );
};

export default QuickNavigation;
